...
public abstract class Unit {
	...
	// Specifying the "player" parameter with 1 means it's the player's unit,
	// any other number means
	// it's the AI
	...
	public Unit(ArrayList<String> units, String UnitType, int player) {
		for (String x : units) {
			if (x.contains(UnitType)) {
				...
				this.setStrength(
						Double.parseDouble(temp.substring(temp.indexOf("<strength>") + 8, temp.indexOf("/n"))));
				temp = temp.substring(temp.indexOf("/n") + 1);
	...
	// constructor 2
	public Unit(boolean player, int strength, double health, int price, int speed, int range, Image img) {
		this.image = img;
		this.player = player;
